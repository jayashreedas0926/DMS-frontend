...
const handleSubmit = (values, mode) => {
  ...
  const handleAddClick = () => {
    form.resetFields();
    form.setFieldsValue({ status: "Pending", returnDate: dayjs() });
      ...
      <Form.Item label="Return Reason" name="returnReason" rules={[{ required: true }]}>
        <Select
          ...
          onChange={(value) => {
            ...
            if (value !== "Other") {
              form.setFieldsValue({ otherReasonText: "" });
            }
          }}
        >
      ...
      render: (record) => (
        <div className="flex gap-3">
          ...
          {record.status!=="Approved" && (
            ...
            onClick={() => {
              setSelectedRecord(record);
              setFormValues(record, form, "edit");
  ...
  return (
    <div>
     ...
     {/* Edit Modal */}
<Modal
  title={<span className="text-amber-700 text-2xl font-semibold">Edit Sale Return</span>}
...
{/* Add Modal */}
<Modal
  title={<span className="text-amber-700 text-2xl font-semibold">Add New Sale Retur</span>}
  open={isAddModalOpen}
